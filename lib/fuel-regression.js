...
async function runPooledComparison(supabase, userId) {
  ...
  var assetsResult = await supabase
    .from('assets')
    .select('id, asset_name, asset_type, travel_rate_lpk, idle_burn_rate_lph')
    .eq('user_id', userId)
    .eq('is_ignored', false);
...
async function runForUser(supabase, userId, dryRun) {
  ...
  var assetSelect = 'id, asset_name, asset_type, travel_rate_lpk, idle_burn_rate_lph, idle_anomaly_confirmed';
