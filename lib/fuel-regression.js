 ...
 *     Runs the synthetic sanity checks. Touches no real data.
 *
...
function runSelfTest() {
  ...
  // Mixed-source fill sequence: card purchases + manual bulk-tank records
  // must interleave chronologically. End-fill litres are unchanged from a
  // single-source sequence with the same dates/litres.
  var mixedFills = mergeFillEvents(
    [
      { purchase_date: '2026-01-01', litres: 100 },
      { purchase_date: '2026-01-15', litres: 90 }
    ],
    [
      { record_date: '2026-01-08', litres: 80 },
      { record_date: '2026-01-22', litres: 70 }
    ]
  );
  var mixedIv = buildIntervals(mixedFills);
  check('Mixed sources: 3 fill-to-fill intervals', mixedIv.length === 3, 'got ' + mixedIv.length);
  check(
    'Mixed sources: chronological interleave purchase→record→purchase→record',
    mixedIv[0].startDate === '2026-01-01' && mixedIv[0].endDate === '2026-01-08' &&
      mixedIv[1].startDate === '2026-01-08' && mixedIv[1].endDate === '2026-01-15' &&
      mixedIv[2].startDate === '2026-01-15' && mixedIv[2].endDate === '2026-01-22',
    JSON.stringify(mixedIv.map(function(iv) { return iv.startDate + '→' + iv.endDate; }))
  );
  check(
    'Mixed sources: end-fill litres match a single-source sequence',
    mixedIv[0].litres === 80 && mixedIv[1].litres === 90 && mixedIv[2].litres === 70,
    JSON.stringify(mixedIv.map(function(iv) { return iv.litres; }))
  );
  var junkDropped = mergeFillEvents(
    [{ purchase_date: '2026-01-01', litres: 100 }],
    [{ record_date: '2026-01-08', litres: 1 }]
  );
  check(
    'Mixed sources: sub-' + MIN_PLAUSIBLE_FILL_LITRES + 'L fuel_records rows are dropped before sequencing',
    junkDropped.length === 1 && junkDropped[0].litres === 100,
    JSON.stringify(junkDropped)
  );
...
/**
 * Card purchases plus manual bulk-tank fuel_records, same shape
 * ({ purchase_date, litres }). Same-day events from both sources stay as
 * two sequence points — buildIntervals + mergeOrphanedLitresForward already
 * treat a genuine double-fill that way (audit upsert even comments on
 * coincident dates from different sources). Do not pre-sum them.
 *
 * MIN_PLAUSIBLE_FILL_LITRES is applied here so a 1L junk row cannot split
 * a real interval. evaluateInterval still re-checks as defence in depth.
 *
 * Machinery fuel_records must never be passed in — callers gate on
 * FCM.isOnRoad. This engine calibrates on-road fill-to-fill only.
 */
function mergeFillEvents(purchaseRows, recordRows) {
  var events = [];
  (purchaseRows || []).forEach(function(r) {
    var date = r.purchase_date ? String(r.purchase_date).slice(0, 10) : '';
    events.push({ purchase_date: date, litres: parseFloat(r.litres) || 0 });
  });
  (recordRows || []).forEach(function(r) {
    var date = r.record_date ? String(r.record_date).slice(0, 10) : '';
    events.push({ purchase_date: date, litres: parseFloat(r.litres) || 0 });
  });
  return events.filter(function(e) {
    return e.purchase_date && e.litres >= MIN_PLAUSIBLE_FILL_LITRES;
  });
}

/**
 * Builds fill-to-fill intervals from a merged fill sequence (purchases
 * and/or bulk-tank records), sorted chronologically. Each interval spans
 * two consecutive fills; litres used are the fill AT THE END.
 */
...
function calibrateAsset(intervals, previousTravelRate) {
  ...
  return {
    ...
    idleLitresTotal: idleLitresTotal,
...
function isIntermittentAsset(asset) {
...
var MIN_IDLE_HOURS_TO_STORE = 20;

function idleStoreDecision(asset, result) {
  if (!result || !result.calibrated) {
    return { store: false, reason: 'not_calibrated' };
  }
  if (isIntermittentAsset(asset)) {
    return { store: false, reason: 'intermittent' };
  }
  if (result.totalIdleHours == null || result.totalIdleHours < MIN_IDLE_HOURS_TO_STORE) {
    return {
      store: false,
      reason: 'insufficient_idle_hours',
      totalIdleHours: result.totalIdleHours
    };
  }
  var idleClass = idleClassForAsset(asset && asset.asset_name);
  ...
  if (result.idleRateLph == null || result.idleRateLph < bounds[0] || result.idleRateLph > bounds[1]) {
    return {
      store: false,
      reason: 'out_of_band',
      idleRateLph: result.idleRateLph,
      bounds: bounds
    };
  }
  return { store: true, idleRateLph: result.idleRateLph, idleClass: idleClass, bounds: bounds };
}
...
async function runPooledComparison(supabase, userId) {
  ...
  var assetsResult = await supabase
    .from('assets')
    .select('id, asset_name, asset_type, is_on_road, travel_rate_lpk, idle_burn_rate_lph')
    .eq('user_id', userId)
    .eq('is_ignored', false);
  ...
  for (var i = 0; i < assets.length; i++) {
    ...
    if (FCM.isOnRoad(asset)) {
      var recordsResult = await supabase
        .from('fuel_records')
        .select('record_date, litres')
        .eq('asset_id', asset.id)
        .order('record_date', { ascending: true });
      if (recordsResult.error) { console.log(asset.asset_name + ': ERROR loading fuel_records'); continue; }
    ...
    var rawIntervals = buildIntervals(mergeFillEvents(fuelResult.data || [], recordRows));
...
async function runForUser(supabase, userId, dryRun) {
  var hasIdleSource = await columnExists(supabase, 'assets', 'idle_rate_source');
  var hasUsage = await columnExists(supabase, 'assets', 'usage_profile');
  var assetSelect = 'id, asset_name, asset_type, is_on_road, travel_rate_lpk, idle_burn_rate_lph, idle_anomaly_confirmed';
  if (hasUsage) assetSelect += ', usage_profile';
  ...
  for (var i = 0; i < assets.length; i++) {
    ...
    if (FCM.isOnRoad(asset)) {
      ...
      if (recordsResult.error) {
        console.log(asset.asset_name + ': ERROR loading fuel_records — ' + recordsResult.error.message);
        continue;
      }
      recordRows = recordsResult.data || [];
    }
    ...
    if (!result.calibrated) {
      ...
      var idleDecision = idleStoreDecision(asset, result);
      if (!anomalous && !idleDecision.store) {
        if (idleDecision.reason === 'intermittent') {
          console.log('  -> idle residual not stored: intermittent/on-call usage (km and idle hours are not separable)');
        } else if (idleDecision.reason === 'insufficient_idle_hours') {
          console.log(
            '  -> idle residual not stored: only ' +
            (idleDecision.totalIdleHours != null ? idleDecision.totalIdleHours.toFixed(1) : 'n/a') +
            ' idle hours in included intervals (need ≥' + MIN_IDLE_HOURS_TO_STORE + ')'
          );
        } else if (idleDecision.reason === 'out_of_band') {
          console.log(
            '  -> idle ' +
            (idleDecision.idleRateLph != null ? idleDecision.idleRateLph.toFixed(3) : 'n/a') +
            ' L/hr is outside the plausible ' + idleDecision.bounds[0] + '–' + idleDecision.bounds[1] +
      ...
      if (!dryRun) {
        ...
        } else if (idleDecision.store) {
          updatePayload.idle_burn_rate_lph = result.idleRateLph;
          if (hasIdleSource) updatePayload.idle_rate_source = 'calibrated';
        } else {
          // Clear leftover seed/default so the UI cannot label 3.00 L/hr
          // as Calibrated from last_calibrated_at + a class default.
          updatePayload.idle_burn_rate_lph = null;
          if (hasIdleSource) updatePayload.idle_rate_source = null;
        }
...
module.exports = {
  nnlsRegressionThroughOrigin: nnlsRegressionThroughOrigin,
  calibrateAsset: calibrateAsset,
  mergeFillEvents: mergeFillEvents,
  buildIntervals: buildIntervals,
  evaluateInterval: evaluateInterval,
  mergeOrphanedLitresForward: mergeOrphanedLitresForward,
  idleClassForAsset: idleClassForAsset,
  isIntermittentAsset: isIntermittentAsset,
  idleStoreDecision: idleStoreDecision,
  IDLE_CLASS_TABLE: IDLE_CLASS_TABLE,
  MIN_INTERVALS_TO_CALIBRATE: MIN_INTERVALS_TO_CALIBRATE,
  MIN_IDLE_HOURS_TO_STORE: MIN_IDLE_HOURS_TO_STORE
};
